  ...
  const handleSignup = (e) => {
    ...
    auth
      ...
      .then((cred) => {
        ...
        // PUSHING Client DATA IN DB
        const clientRef = db.doc(`clients/${user.uid}`);
        clientRef.set({
          name,
          email,
          imageURL: null,
        ...
        db.doc(`clients/${cred.user.uid}`).collection("notifications").add({
  ...
  const signInWithGoogle = () => {
    ...
    firebase
      ...
      .then((cred) => {
        ...
        // PUSHING client DATA IN DB
        const clientRef = db.doc(`clients/${cred.user.uid}`);
        clientRef.set({
          name: cred.user.displayName,
          email: cred.user.email,
          uid: cred.user.uid,
          ...
          <Box sx={box}>
            ...
            <Box component="form" noValidate onSubmit={handleSignup}>
              ...
              <Grid container justifyContent="flex-end">
                <Grid item>
                  <Link to={"/clientsignin"} variant="body2">
